    ...
    public function index()
    {
        ...
        $stats = [
            ...
            'recent_assignments' => Assignment::with(['user', 'tournament'])
                ->when($user->isZoneAdmin() && $user->zone_id, fn ($q) => $q->whereHas('tournament.club', fn ($sub) => $sub->where('zone_id', $user->zone_id)))
                ->when($user->user_type === UserType::NationalAdmin, fn ($q) => $q->whereHas('tournament.tournamentType', fn ($sub) => $sub->where('is_national', true)))
                ->latest()
                ->limit(5)
                ->get(),
        ];
