                            ...
                            <div class="flex items-center">
                                ...
                                <div class="ml-4">
                                    <p class="text-2xl font-bold text-gray-900">
                                        {{ number_format($stats['size_total'] / 1024 / 1024, 1) }} MB</p>
                                ...
                                <div>
                                    ...
                                    <select name="category"
                                        ...
                                        <option value="tournament"
                                            {{ request('category') === 'tournament' ? 'selected' : '' }}>Torneo</option>
                                        <option value="regulation"
                                            {{ request('category') === 'regulation' ? 'selected' : '' }}>Regolamento
                                        </option>
                                        <option value="form" {{ request('category') === 'form' ? 'selected' : '' }}>Modulo
                                        </option>
                                        <option value="template" {{ request('category') === 'template' ? 'selected' : '' }}>
                                            Template</option>
                                    </select>
                                </div>
                                ...
                                <div>
                                    ...
                                    <select name="type"
                                        ...
                                        <option value="spreadsheet"
                                            {{ request('type') === 'spreadsheet' ? 'selected' : '' }}>Foglio calcolo
                                        </option>
                                        ...
                                        @foreach ($documents as $document)
                                            <tr class="hover:bg-gray-50">
                                                <td class="px-6 py-4 whitespace-nowrap">
                                                    <div class="flex items-center">
                                                        @php
                                                            ...
                                                            $iconClass =
                                                                $iconClasses[$document->type] ?? 'text-gray-400';
                                                        @endphp
                                                        <svg class="w-5 h-5 {{ $iconClass }} mr-3 flex-shrink-0"
                                                ...
                                                <td class="px-6 py-4 whitespace-nowrap">
                                                    ...
                                                    @endphp
                                                    <span
                                                        class="inline-flex px-2 py-1 text-xs font-semibold rounded-full {{ $categoryColors[$document->category] ?? 'bg-gray-100 text-gray-800' }}">
                                                        {{ \App\Models\Document::CATEGORIES[$document->category] }}
                                                    </span>
                                                    @if ($document->is_public)
                                                        <div class="mt-1">
                                                            <span
                                                                class="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-green-100 text-green-800">Pubblico</span>
                                                        </div>
                                                    @endif
                                                </td>
                                                <td class="px-6 py-4 whitespace-nowrap">
                                                    <div class="text-sm text-gray-900">{{ $document->file_size_human }}
                                                    </div>
                                                    @if ($document->download_count > 0)
                                                        <div class="text-sm text-blue-600">{{ $document->download_count }}
                                                            download</div>
                                                    @endif
                                                </td>
                                                <td class="px-6 py-4 whitespace-nowrap">
                                                    <div class="text-sm text-gray-900">{{ $document->uploader->name }}
                                                    </div>
                                                    @if ($document->zone)
                                                        <div class="text-sm text-gray-500">{{ $document->zone->name }}
                                                        </div>
                                                    @endif
                                                </td>
                                                <td class="px-6 py-4 whitespace-nowrap">
                                                ...
                                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                    <div class="flex items-center justify-end gap-2">
                                                        ...
                                                        @if (auth()->user()->id === $document->uploader_id || auth()->user()->isSuperAdmin())
                                                            <form method="POST"
                ...
                <div id="uploadModal"
                    ...
                    <div
                        ...
                        <div class="mt-3">
                            ...
                            <form method="POST" action="{{ route('admin.documents.upload') }}"
                                ...
                                <div>
                                    ...
                                    <textarea name="description" rows="3" placeholder="Descrizione opzionale del documento..."
