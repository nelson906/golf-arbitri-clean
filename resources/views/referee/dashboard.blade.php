                    ...
                    <div class="bg-purple-50 p-6 rounded-lg">
                        <div class="flex items-center">
                            <div class="flex-1">
                                ...
                                <p class="text-lg font-bold text-purple-900">{{ ucfirst($user->level ?? 'Non specificato') }}</p>
                            </div>
                    ...
                    <div class="bg-yellow-50 p-6 rounded-lg">
                        <div class="flex items-center">
                            <div class="flex-1">
                                ...
                                <p class="text-lg font-bold text-yellow-900">{{ $user->zone->name ?? 'Non assegnata' }}</p>
                            </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                        @foreach($openTournaments->take(6) as $tournament)
                            <div class="bg-white border rounded-lg p-3">
                                ...
                                <p class="text-xs text-amber-600 mt-1">Scadenza: {{ $tournament->availability_deadline?->format('d/m/Y') ?? 'N/A' }}</p>
                            </div>
                        @endforeach
                    </div>
                    @if($openTournaments->count() > 6)
                        <div class="mt-3 text-center">
                            <a href="{{ route('user.availability.tournaments') }}" class="text-amber-700 hover:text-amber-900 text-sm">
                                ...
                                @foreach($upcomingAssignments as $assignment)
                                    <div class="border rounded-lg p-4 hover:bg-gray-50">
                                        <div class="flex justify-between items-start">
                                            ...
                                            <span class="px-2 py-1 text-xs rounded-full {{ $assignment->is_confirmed ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800' }}">
                            ...
                            <div class="space-y-3">
                                @foreach($pendingAvailabilities as $availability)
                                    <div class="border rounded-lg p-4 hover:bg-gray-50">
                                        ...
                                        <p class="text-sm text-gray-500">{{ $availability->tournament->club->name ?? 'N/A' }}</p>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                        @foreach($recentAssignments as $assignment)
                            <div class="border rounded-lg p-3 bg-gray-50">
                                ...
                                <p class="text-xs text-gray-500">{{ $assignment->tournament->club->name ?? 'N/A' }}</p>
                            </div>
                        @endforeach
                    </div>
                </div>
