...
@section('content')
    <div class="container mx-auto px-4 py-6">
        ...
        <div class="bg-white rounded-lg shadow p-6">
            <form method="POST" action="{{ route('admin.letter-templates.update', $template) }}" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="type" id="type"
                            ...
                            <option value="assignment"
                                {{ old('type', $template->type) === 'assignment' ? 'selected' : '' }}>
                                Assegnazione
                            </option>
                            <option value="convocation"
                                {{ old('type', $template->type) === 'convocation' ? 'selected' : '' }}>
                                Convocazione
                            </option>
                            ...
                            <option value="institutional"
                                {{ old('type', $template->type) === 'institutional' ? 'selected' : '' }}>
                                Istituzionale
                            </option>
                        </select>
                        @error('type')
                        ...
                        <select name="zone_id" id="zone_id"
                            ...
                            @foreach ($zones as $zone)
                                <option value="{{ $zone->id }}"
                                    {{ old('zone_id', $template->zone_id) == $zone->id ? 'selected' : '' }}>
                                    {{ $zone->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    {{-- Tipo Torneo --}}
                    <div>
                        <label for="tournament_type_id" class="block text-sm font-medium text-gray-700 mb-1">Tipo
                            Torneo</label>
                        <select name="tournament_type_id" id="tournament_type_id"
                            ...
                            @foreach ($tournamentTypes as $type)
                                <option value="{{ $type->id }}"
                                    {{ old('tournament_type_id', $template->tournament_type_id) == $type->id ? 'selected' : '' }}>
                                    {{ $type->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div>
                    ...
                    <input type="text" name="subject" id="subject" value="{{ old('subject', $template->subject) }}"
                        ...
                        placeholder="Es: Assegnazione {{ 'tournament_name' }} - {{ 'assigned_date' }}" required>
                    @error('subject')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <textarea name="body" id="body" rows="15"
                        ...
                        placeholder="Scrivi il contenuto del template qui... Usa {{ 'variabile' }} per i dati dinamici." required>{{ old('body', $template->body) }}</textarea>
                    @error('body')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
