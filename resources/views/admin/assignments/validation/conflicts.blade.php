                ...
                <div class="flex items-center justify-between">
                    <div class="flex items-center">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-gray-600">Il sistema può tentare di risolvere automaticamente alcuni conflitti sostituendo gli arbitri con alternative valide</p>
                        </div>
                    </div>
                    <form action="{{ route('admin.assignment-validation.fix-conflicts') }}" method="POST" onsubmit="return confirm('Vuoi procedere con la risoluzione automatica? Alcune assegnazioni verranno modificate.')">
                        @csrf
                ...
                <div class="p-6">
                        ...
                        <div class="flex items-start space-x-3">
                            <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium
                                {{ $conflict['severity'] === 'high' ? 'bg-red-100 text-red-800' :
                                   ($conflict['severity'] === 'medium' ? 'bg-yellow-100 text-yellow-800' : 'bg-blue-100 text-blue-800') }}">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        <div class="bg-blue-50 border-l-4 border-blue-400 p-4 rounded">
                            ...
                            <p class="font-medium text-gray-900">{{ $conflict['assignment1']->tournament->name }}</p>
                            ...
                            <span class="inline-flex items-center px-2 py-1 rounded text-xs font-medium bg-blue-100 text-blue-800 mt-2">
                        ...
                        <div class="bg-red-50 border-l-4 border-red-400 p-4 rounded">
                            ...
                            <p class="font-medium text-gray-900">{{ $conflict['assignment2']->tournament->name }}</p>
                            <p class="text-sm text-gray-600 mt-1">
                                <svg class="inline w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/>
                                </svg>
                            ...
                            <span class="inline-flex items-center px-2 py-1 rounded text-xs font-medium bg-red-100 text-red-800 mt-2">
                                    ...
                                    @foreach($suggestion['alternative_referees'] as $altReferee)
                                        <a href="{{ route('admin.assignments.edit', $conflict['assignment2']->id) }}?suggested_referee={{ $altReferee->id }}"
                                           class="inline-flex items-center px-3 py-1 border border-blue-300 rounded-md text-sm font-medium text-blue-700 bg-white hover:bg-blue-50">
                                            {{ $altReferee->name }}
                                            <span class="ml-2 px-2 py-0.5 rounded text-xs bg-blue-100">{{ ucfirst($altReferee->level) }}</span>
                                        </a>
                                    @endforeach
                                </div>
                            @else --}}
