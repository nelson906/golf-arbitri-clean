            ...
            <div class="bg-white shadow-sm rounded-lg p-6">
                        ...
                        <select name="tournament_type_id" id="tournament_type_id" required
                            ...
                            @foreach ($tournamentTypes as $type)
                                <option value="{{ $type->id }}"
                                    {{ old('tournament_type_id', $tournament->tournament_type_id) == $type->id ? 'selected' : '' }}>
                                    {{ $type->name }}
                                </option>
                            @endforeach
                        </select>
                            ...
                            <select name="zone_id" id="zone_id" required
                                ...
                                @foreach ($zones as $zone)
                                    <option value="{{ $zone->id }}"
                                        {{ old('zone_id', $tournament->zone_id) == $zone->id ? 'selected' : '' }}>
                                        {{ $zone->name }}
                                    </option>
                                @endforeach
                            </select>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div>
                        ...
                        <input type="date" name="availability_deadline" id="availability_deadline"
                            value="{{ old('availability_deadline', Carbon\Carbon::parse($tournament->availability_deadline)->format('Y-m-d')) }}"
                            required
                            class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
