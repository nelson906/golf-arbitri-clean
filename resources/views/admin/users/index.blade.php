...
<div class="py-6">
    ...
    <div class="bg-white rounded-lg shadow mb-6">
        <div class="p-4">
            <form method="GET" action="{{ route('admin.users.index') }}" class="space-y-4 md:space-y-0 md:flex md:gap-4">
                {{-- Ricerca --}}
                <div class="flex-1">
                    <input type="text"
                           name="search"
                ...
                {{-- Tipo Utente --}}
                <div>
                    <select name="user_type" class="rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                        <option value="">Tutti i tipi</option>
                        <option value="referee" {{ request('user_type') == 'referee' ? 'selected' : '' }}>Arbitro</option>
                        <option value="admin" {{ request('user_type') == 'admin' ? 'selected' : '' }}>Admin Zona</option>
                        @if(isset($isNationalAdmin) && $isNationalAdmin)
                ...
                @if(isset($isNationalAdmin) && $isNationalAdmin && isset($zones))
                    <div>
                        <select name="zone_id" class="rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                ...
                @forelse($users as $user)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            @endphp
                            <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $typeColors[$user->user_type] ?? 'bg-gray-100 text-gray-800' }}">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div class="flex justify-end gap-2">
                                ...
                                {{-- Modifica --}}
                                @if(
                                    (isset($isNationalAdmin) && $isNationalAdmin) ||
                                    (auth()->user()->zone_id == $user->zone_id)
                                )
