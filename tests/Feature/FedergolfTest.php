...
/**
 * Copre App\Http\Controllers\User\FedergolfController.
 *
...
class FedergolfTest extends TestCase
{
    ...
    public function test_load_all_competitions_detects_tipo_from_name(): void
    {
        ...
        $gare = [];
        foreach ($this->jsonArray($response, 'gare') as $gara) {
            $this->assertIsArray($gara);
            $id = $gara['id'] ?? null;
            // L'id resta com'e' lo manda federgolf (qui int): non lo normalizziamo.
            $this->assertIsInt($id);
            $gare[$id] = $gara;
        }
